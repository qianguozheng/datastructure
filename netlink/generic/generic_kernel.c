...
static int __init genKernel_init(void) {
         ...
         rc =  _genl_register_family_with_ops_grps(&doc_exmpl_genl_family, &doc_exmpl_genl_ops_echo, 1, NULL, 0);
         if (rc != 0) {
			 printk("Register ops: %i\n",rc);
			 //genl_unregister_family_with_ops_groups(&doc_exmpl_genl_family);
			goto failure;
         }
         return 0;
