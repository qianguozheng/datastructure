...
//https://stackoverflow.com/questions/13227315/udp-packet-send-with-linux-kernel-module-without-using-sockets
//
MODULE_LICENSE("GPL");
             ...
             u_short sport, u_short dport)
{
  ...
  printk("skb_push udp header start\n");
  udp_len = sizeof(struct udphdr)+strlen(app_data);
	p = skb_push(skb, sizeof(struct udphdr));
  {
    ...
    skb->csum = 0;
    udph->check = 0;
    udph->check = csum_tcpudp_magic(ntohs(sip), ntohs(dip), 
									udp_len,IPPROTO_UDP, 
									csum_partial(udph, udp_len, 0));
	if (udph->check == 0){
		printk("udph->check is 0, reset it\n");
		udph->check = CSUM_MANGLED_0;
	}
  }
  printk("skb_push udp header done\n");
